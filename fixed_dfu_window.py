#!/usr/bin/env python
"""
Fixed DFU tool for ReSpeaker USB Mic Array
수정된 버전: 이미 DFU 모드에 있을 때 바로 펌웨어 다운로드 진행

Requirements:
    pip install pyusb click

Usage:
    python fixed_dfu_windows.py --download new_firmware.bin
    python fixed_dfu_windows.py --revertfactory
"""
    ...
    def download(self, firmware):
        ...
        while True:
            ...
            if block_number % 100 == 0:
                print(f'Downloaded {block_number * block_size} bytes...')
...
def main(download, revertfactory):
    try:
        ...
        print(f"❌ Error: {e}")
        print("💡 Try manually entering DFU mode first")
    except Exception as e:
